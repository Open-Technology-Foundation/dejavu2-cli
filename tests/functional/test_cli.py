...
CLI_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "dejavu2-cli")
...
class TestDejavu2CLI:
    ...
    def test_list_models(self):
        ...
        result = subprocess.run([CLI_PATH, "--list-models"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_list_templates(self):
        ...
        result = subprocess.run([CLI_PATH, "--list-template-names"], capture_output=True, text=True)
        assert result.returncode == 0
    ...
    def test_basic_query(self, reference_file):
        ...
        result = subprocess.run(
            [CLI_PATH, "What is the capital of France?", "-r", str(reference_file)],
    ...
    def test_template_query(self):
        ...
        result = subprocess.run(
            [CLI_PATH, "What is the capital of France?", "-T", "Dejavu2 - Helpful AI"],
            capture_output=True, text=True
        )
        assert result.returncode == 0
        assert "Paris" in result.stdout