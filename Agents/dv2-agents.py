...
def list_categories(data: dict[str, Any]):
  ...
  categories = {agent["category"] for agent in data.values()}
  if categories:
...
def XXXedit_agent(data: dict[str, Any], key: str):
  ...
  # Convert to YAML
  agent_yaml = yaml.dump({full_key: data[full_key]}, default_flow_style=False, allow_unicode=True, width=1000)
...
def main():
  ...
  if args.command == "list":
    ...
    kwargs = {k: v for k, v in vars(args).items() if k not in ["command", "key"] and v is not None}
