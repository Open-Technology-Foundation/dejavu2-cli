...
# Lazy imports moved into functions for performance:
# - time: only needed in handle_file_output and write_combined_output_file
# - xml.sax.saxutils: only needed in execute_queries
# - datetime: only needed in handle_conversation_listing
# - post_slug: only needed in prepare_query_execution and handle_file_output
# - config, context, conversations, display: imported when needed
# - llm_clients, models, templates: imported when needed
# - utils.spacetime_placeholders: imported when needed

from errors import ConfigurationError, ConversationError, KnowledgeBaseError, ModelError, ReferenceError, TemplateError
...
def setup_application(kwargs: dict[str, Any]) -> tuple:
  ...
  logger = setup_logging(verbose=verbose, log_file=kwargs["log_file"], quiet=quiet)

  # Create module-specific loggers for other modules
  config_logger = logging.getLogger("config")
  logging.getLogger("models")
  logging.getLogger("templates")
  logging.getLogger("context")
  logging.getLogger("llm_clients")
  logging.getLogger("display")
  logging.getLogger("conversations")
...
def handle_utility_commands(kwargs: dict[str, Any], paths: dict[str, str]) -> bool:
  ...
  """
  # Lazy imports
  if kwargs["edit_templates"] or kwargs["edit_defaults"] or kwargs.get("edit_models"):
    from config import edit_json_file, edit_yaml_file

  if kwargs["list_template"] or kwargs["list_template_names"]:
    from templates import list_templates, list_template_names

  if kwargs["list_models"] or kwargs["list_models_details"]:
...
def handle_conversation_listing(conv_manager) -> None:
  """
...
def handle_conversation_deletion(conv_manager: Any, conversation_id: str) -> None:
  """
...
def handle_message_operations(kwargs: dict[str, Any], conv_manager: Any) -> bool:
  """
  ...
  if kwargs["list_messages"]:
    ...
    for msg in messages:
      ...
      click.echo("{:<5} {:<10} {:<20} {:<50}".format(idx, role, msg["timestamp"], msg["content_preview"]))
    return True
...
def handle_conversation_export(kwargs: dict[str, Any], conv_manager: Any) -> None:
  """
...
def prepare_query_execution(
  kwargs: dict[str, Any], config: dict[str, Any], paths: dict[str, str], conv_manager
) -> dict[str, Any]:
  """
  ...
  from llm_clients import get_api_keys, initialize_clients
  ...
  reference_string, knowledgebase_string = process_reference_and_knowledge(kwargs, paths, api_keys, knowledgebase_query)
...
def setup_active_conversation(kwargs: dict[str, Any], conv_manager):
  """
  ...
  if (active_conversation is None) or kwargs["new_conversation"]:
    ...
    active_conversation = conv_manager.new_conversation(system_prompt=system_prompt, title=kwargs.get("title"), metadata=conversation_metadata)
...
def setup_model_and_clients(kwargs: dict[str, Any], models_json_path: str) -> tuple:
  ...
  """
  # Lazy import
  from models import get_canonical_model
...
def process_reference_and_knowledge(kwargs: dict[str, Any], paths: dict[str, str], api_keys: dict[str, str], knowledgebase_query: str) -> tuple:
  ...
  if kwargs["knowledgebase"]:
    try:
      ...
      try:
        knowledgebase_string = get_knowledgebase_string(kb_path, knowledgebase_query, paths["customkb_executable"], paths["vectordbs_path"], api_keys)
      except KnowledgeBaseError as e:
        error_msg = f"Knowledgebase error: {e}"
        if bypass_kb_errors:
          click.echo(f"Warning: {error_msg} (continuing without knowledgebase)", err=True)
          knowledgebase_string = "<knowledgebase>\n# Error querying knowledgebase (continuing without it)\n</knowledgebase>\n\n"
        else:
...
# QUERY CONFIGURATION =====================================================================
@click.option("-T", "--template", default=None, help='The template name to initialize arguments from (eg, "Helpful_AI")')
@click.option("-m", "--model", default=None, help='The LLM model to use (eg, "gpt-4")')
@click.option("-s", "--systemprompt", default=None, help='The system role for the AI assistant (eg, "You are a helpful assistant.")')
@click.option("-t", "--temperature", type=float, default=None, help="The sampling temperature for the LLM (eg, 0.7)")
@click.option("-M", "--max-tokens", type=int, default=None, help="The maximum number of tokens for the LLM (eg, 1000)")
# CONTEXT & REFERENCES ====================================================================
@click.option("-r", "--reference", default=None, help="A comma-delimited list of text files for inclusion as context before the query")
@click.option("-k", "--knowledgebase", default=None, help='The knowledgebase for the query (eg, "my_knowledge_base")')
@click.option("-Q", "--knowledgebase-query", default=None, help="Query to be sent to the knowledgebase instead of the command-line query")
# INFORMATION & LISTING ===================================================================
@click.option("-S", "--status", is_flag=True, default=False, help="Display the state of all arguments and exit")
@click.option("-a", "--list-models", is_flag=True, default=False, help="List all available models from Models.json")
@click.option("-A", "--list-models-details", is_flag=True, default=False, help="List all available models from Models.json, and display all elements")
@click.option("-l", "--list-template", default=None, help='List all templates, or a specific template (eg, "all" or "Helpful_AI")')
@click.option("-L", "--list-template-names", is_flag=True, default=False, help="List the templates, without the systemprompt")
@click.option("-K", "--list-knowledge-bases", is_flag=True, default=False, help="List all available knowledgebases")
@click.option("-P", "--print-systemprompt", is_flag=True, default=False, help="Print the full system prompt when using --status")
# CONFIGURATION EDITING ===================================================================
@click.option("-E", "--edit-templates", is_flag=True, default=False, help="Edit Agents.json file")
@click.option("-D", "--edit-defaults", is_flag=True, default=False, help="Edit defaults.yaml file")
@click.option("-d", "--edit-models", is_flag=True, default=False, help="Edit Models.json")
# CONVERSATION MANAGEMENT =================================================================
@click.option("-c", "--continue", "continue_conv", is_flag=True, default=False, help="Continue the most recent conversation")
@click.option("--conversation", "-C", "conversation_id", type=str, help="Load a specific conversation by ID")
@click.option("--new-conversation", "-n", is_flag=True, default=False, help="Start a new conversation even when continuing would be possible")
@click.option("--title", "-i", type=str, help="Set a title for a new conversation")
@click.option("--list-conversations", "-x", is_flag=True, default=False, help="List all saved conversations")
@click.option("--delete-conversation", "-X", type=str, help="Delete a specific conversation by ID")
@click.option(
  "--list-messages", "-W", type=str, metavar="CONVERSATION_ID", help="List all messages in a conversation with their indices and content previews"
)
@click.option(
  "--remove-message",
  nargs=2,
  type=(str, int),
  metavar=("CONVERSATION_ID", "MESSAGE_INDEX"),
...
@click.option(
  ...
  nargs=2,
  type=(str, int),
  metavar=("CONVERSATION_ID", "USER_MESSAGE_INDEX"),
  help="Remove a user-assistant message pair (index must point to a user message followed by an assistant message)",
)
@click.option("--export-conversation", "-e", type=str, help='Export a conversation to markdown (specify ID or "current")')
@click.option("--export-path", "-f", type=str, help="Path to save the exported conversation markdown file")
# OUTPUT OPTIONS ==========================================================================
@click.option("-p", "--project-name", default=None, help='The project name for recording conversations (eg, -p "bali_market")')
@click.option("-o", "--output-dir", default=None, help='Directory to output results to (eg, "/tmp/myfiles")')
@click.option(
  "-g", "--message", type=(str, str), multiple=True, help='Add message pairs in the form: -g role "message" (eg, -g user "hello" -g assistant "hi")'
)
@click.option("--stdout", "-O", is_flag=True, default=False, help="Output the exported conversation to stdout instead of a file")
# LOGGING & DEBUG =========================================================================
@click.option("-v", "--verbose", is_flag=True, default=False, help="Enable verbose (debug level) logging")
@click.option("--log-file", type=str, help="Path to a log file where all logs will be written")
@click.option("-q", "--quiet", is_flag=True, default=True, help="Suppress log messages except for errors")
...
def main(**kwargs: Any) -> None:
  ...
  if kwargs["status"]:
    display_status(
      kwargs,
      query_context["query_texts"],
      config,
      query_context["model_parameters"],
      kwargs.get("print_systemprompt", False),
      query_context["active_conversation"],
    )
    return
...
def execute_queries(query_context: dict[str, Any], conv_manager) -> None:
  """
...
def handle_conversation_response(query_context: dict[str, Any], conv_manager, query_result: str) -> None:
  """
  ...
  if active_conversation:
    ...
    if (not active_conversation.title or active_conversation.title == "Untitled Conversation") and len(active_conversation.messages) >= 3:
      ...
      title = conv_manager.suggest_title_from_content(active_conversation, simple_title_query)
...
if __name__ == "__main__":
  # Early exit for --version and --help before Click processes everything
  # This saves ~100ms by avoiding full Click initialization
  if len(sys.argv) >= 2:
    arg = sys.argv[1]
    ...
    if arg in ["--version", "-V"]:
      try:
        ...
        VERSION = "unknown"
      print(f"dejavu2-cli v{VERSION}")
      sys.exit(0)
    # Handle --help and -h flags
    elif arg in ["--help", "-h"]:
      # Let Click handle this normally since we need its help formatting
      pass
